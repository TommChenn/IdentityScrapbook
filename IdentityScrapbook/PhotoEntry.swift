...
final class PhotoEntry {
    ...
    init(id: UUID, bookID: UUID, voteID: UUID?, caption: String, sequence: Int,
         width: Int, height: Int, now: Date, timeZone: TimeZone) {
        self.id = id
        scrapbookID = bookID
        linkedVoteID = voteID
...
nonisolated enum PhotoFiles {
    ...
    static func preview(_ data: Data, maxPixels: Int = 1600) throws -> Data {
    ...
    static func prepare(original: Data, id: UUID, root: URL) throws -> (width: Int, height: Int) {
        let thumbnail = try preview(original)
        ...
        try thumbnail.write(to: folder.appendingPathComponent("preview.jpg"), options: .atomic)
        return (width, height)
    }
}
...
enum PhotoStore {
    ...
    static func addPhoto(_ original: Data, caption: String, bookID: UUID, voteID: UUID?,
                         ...
                         root: URL = PhotoFiles.root) async throws -> UUID {
        ...
        let size = try await Task.detached {
            try PhotoFiles.prepare(original: original, id: submissionID, root: root)
        }.value
        ...
        let entry = PhotoEntry(id: submissionID, bookID: bookID, voteID: voteID,
                               ...
                               width: size.width, height: size.height, now: Date(), timeZone: .current)
